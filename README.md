...
- **Kakao 모바일 광고**:
  - Kakao 비즈보드, 디스플레이 네이티브, 디스플레이 카탈로그, 상품 카탈로그 1차 지면 지원
  - 레거시 `kakao-mobile-feed` 요청은 `kakao-display-native`로 호환 처리
...
- **2026.05.03**: Kakao 모바일 상품을 비즈보드, 디스플레이 네이티브, 디스플레이 카탈로그, 상품 카탈로그 surface로 분리
