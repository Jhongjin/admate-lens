...
- 현재 공개 폼과 생성 API에서는 내부 프리뷰 타입으로 막혀 있다.
- README에는 Desktop In-feed Home 구현 완료로 설명되어 있어 문서와 공개 운영 상태가 어긋난다.

작업 방향:

1. 내부 구현 결과를 실제 YouTube PC 홈 네이티브 카드 기준으로 QA한다.
2. 공개 상품으로 사용할 수 있으면 폼 옵션, API 허용 목록, 작업 목록 라벨을 함께 정리한다.
3. QA 기준을 통과하지 못하면 README와 운영 문구에서 내부 프리뷰 상태를 명확히 표시한다.
...
| 범위 | 상세 | 상태 |
|---|---|---|
| Google Ads 상품 선택 | 운영자 폼에서 Demand Gen을 선택 가능한 상품으로 정리 | 신규 구현 |
| YouTube Feed | Demand Gen의 YouTube Feed 노출 증빙 | 신규 구현 |
| YouTube Shorts | Demand Gen의 YouTube Shorts 노출 증빙 | 신규 구현 |
| 작업 목록/메타데이터 | Demand Gen 결과임을 작업 이력에서 식별 | 신규 구현 |
...
1. 현재 인스트림 Skip UI 미세조정 변경을 별도 변경 단위로 정리한다.
2. PC 홈 In-feed 내부 구현을 QA하고 공개화 가능 여부를 판단한다.
3. 공개화 가능 시 `infeed-home`을 공개 상품으로 열고 폼/API/목록 라벨을 정리한다.
4. Demand Gen 1차 설계를 적용한다.
5. Demand Gen을 Google Ads 상품 흐름에서 선택 가능하게 만든다.
6. Demand Gen YouTube Feed와 YouTube Shorts 결과를 생성한다.
7. 작업 이력과 결과 메타데이터에서 일반 YouTube 상품과 Demand Gen 상품을 구분한다.
8. Display/Overlay는 레거시 상태를 유지하고, 필요 시 별도 최신 재구현 작업으로 분리한다.
