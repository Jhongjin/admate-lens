...
/** maxDuration(예: 300s) 대비 업로드·DB·Chromium 종료·잔여 pending 정리 여유 */
export const SERVERLESS_BATCH_BUDGET_MS = 270_000;

/** 이보다 적게 남으면 새 캡처를 시작하지 않고 남은 작업을 실패 처리 */
export const MIN_REMAINING_FOR_HARD_SKIP_MS = 45_000;
...
export function resolveBatchPerCaptureTimeoutMs(
  ...
  return Math.min(60_000, Math.max(25_000, remaining - 15_000));
}
